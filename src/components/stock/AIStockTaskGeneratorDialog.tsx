...
interface AIStockTaskGeneratorDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  items: StockItem[];
  categories: StockCategory[];
  stockMode: "expiring" | "calibrating" | "electrical";
  onConfirmTasks: (tasks: Omit<Task, "id" | "createdAt">[]) => void;
}

function generateMockStockTasks(
  mode: "expiring" | "calibrating" | "electrical",
  categories: StockCategory[]
): GeneratedTask[] {
  const staffNames = mockStaffMembers.map((s) => `${s.firstName} ${s.lastName}`);
  const now = Date.now();

  const mockByMode: Record<string, { title: string; catId: string; urgent: boolean }[]> = {
    expiring: [
      { title: "Restock expired Paracetamol 500mg — Drug Cupboard", catId: "drug-cupboard", urgent: true },
      { title: "Replace expired Saline Flush syringes — Consumables", catId: "consumables", urgent: true },
      { title: "Order replacement Flu Vaccines batch — Vaccines", catId: "vaccines", urgent: false },
      { title: "Check and rotate Doctor's Bag adrenaline stock", catId: "doctors-bag", urgent: true },
      { title: "Audit Emergency Trolley medications expiry dates", catId: "emergency-trolley", urgent: false },
      { title: "Review Patient-Owned medication storage compliance", catId: "patient-owned", urgent: false },
    ],
    calibrating: [
      { title: "Schedule blood pressure monitor calibration — Vital Signs", catId: "vital-signs", urgent: true },
      { title: "Book pulse oximeter recalibration service", catId: "vital-signs", urgent: false },
      { title: "Arrange autoclave validation test — Sterilization", catId: "sterilization-infection", urgent: true },
      { title: "Spirometer calibration overdue — Diagnostics", catId: "diagnostic-measurement", urgent: true },
      { title: "Defibrillator maintenance check due — Therapeutic", catId: "therapeutic-treatment", urgent: false },
    ],
    electrical: [
      { title: "Test & tag Treatment Room 1 equipment — overdue", catId: "electrical", urgent: true },
      { title: "Schedule annual electrical safety inspection", catId: "electrical", urgent: false },
      { title: "Re-tag sterilizer power unit — 6 month review", catId: "electrical", urgent: true },
      { title: "Update electrical test register for ECG machine", catId: "electrical", urgent: false },
      { title: "Book electrician for portable appliance testing", catId: "electrical", urgent: false },
    ],
  };

  const items = mockByMode[mode] || mockByMode.expiring;
...
const MODE_CONFIG = {
  expiring: {
    ...
    description: "Scan stock categories for expired and expiring items, then generate restock and audit tasks.",
    scanLabel: "Scanning stock categories for expiry issues...",
  },
  calibrating: {
    icon: Thermometer,
    title: "Calibration",
    description: "Scan calibrating equipment for overdue and upcoming calibrations, then generate service tasks.",
    scanLabel: "Scanning calibration schedules...",
  },
  electrical: {
    icon: Zap,
    title: "Test & Tagging",
    description: "Scan electrical test records for overdue and upcoming tests, then generate testing tasks.",
    scanLabel: "Scanning electrical test records...",
  },
};

export function AIStockTaskGeneratorDialog({
  open,
  onOpenChange,
  items,
  categories,
  stockMode,
  onConfirmTasks,
}: AIStockTaskGeneratorDialogProps) {
  ...
  const config = MODE_CONFIG[stockMode];

  const STEPS = [
    { icon: Search, label: stockMode === "expiring" ? "Scanning stock categories" : stockMode === "calibrating" ? "Reading calibration records" : "Checking test & tag records" },
    { icon: CalendarIcon, label: stockMode === "expiring" ? "Checking expiry dates" : "Analyzing service schedules" },
    { icon: Users, label: "Matching responsible staff" },
    { icon: Bell, label: "Configuring alerts & reminders" },
    { icon: ListChecks, label: "Generating task list" },
  ];
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const tasks = generateMockStockTasks(stockMode, categories);
  ...
  const handleConfirm = () => {
    ...
    toast.success(`${selected.length} task${selected.length > 1 ? "s" : ""} added to task list`);
    setGeneratedTasks([]);
  ...
  const validCount = items.length - expiredCount - expiringCount;
      ...
      <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            AI Task Generator — {config.title}
          </DialogTitle>
        ...
        <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-[13px] space-y-1">
          <p className="font-medium">Stock Summary — {config.title}</p>
        ...
        {!hasGenerated ? (
            ...
            {isGenerating ? (
              ...
              <>
                ...
                <div className="text-center space-y-1">
                  <p className="text-sm font-medium">Ready to scan {config.title.toLowerCase()} items</p>
                  <p className="text-xs text-muted-foreground">
                    AI will read all {config.title.toLowerCase()} records, identify items needing attention,
                    and generate tasks with assigned staff and SMS/email alerts.
                  </p>
                </div>
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-3 animate-scale-in">
            ...
            <p className="text-sm font-medium">All stock items are up to date!</p>
            <p className="text-xs text-muted-foreground">No actions needed at this time.</p>
          </div>
        ) : (
          <>
            {/* Select all / Deselect */}
            <div className="flex items-center justify-between text-xs text-muted-foreground px-1 animate-fade-in">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => toggleAll(false)} className="hover:underline">Deselect all</button>
              </div>
            </div>
                ...
                {generatedTasks.map((task, index) => (
                  <motion.div
                    ...
                    className={`p-3 rounded-lg border transition-all duration-200 ${
                      task.selected ? "border-primary/30 bg-primary/5" : "border-border bg-muted/20 opacity-60"
                    }`}
                  >
