...
interface CreateTaskFromStockDialogProps {
...
export function CreateTaskFromStockDialog({
  stockItem,
  open,
  onOpenChange,
  onCreateTask,
}: CreateTaskFromStockDialogProps) {
  ...
  useEffect(() => {
    if (stockItem && open) {
      const actionPrefix = stockItem.status === "expired" ? "Replace expired" : "Check/Reorder";
      setTitle(`${actionPrefix}: ${stockItem.name}`);
      setDueDate(stockItem.status === "expired" ? new Date() : stockItem.expiryDate);
      setIsImportant(stockItem.status === "expired");
    }
  }, [stockItem, open]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Create Task from Stock Item</DialogTitle>
          <DialogDescription>
            Create a task related to this stock item for tracking and follow-up.
          </DialogDescription>
        </DialogHeader>
